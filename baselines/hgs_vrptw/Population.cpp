...
void Population::doLocalSearchAndAddIndividual(Individual *indiv) {
  ...
  // With a certain probability, repair half of the solutions by increasing the
  // penalties for infeasibilities (w.r.t. capacities and time warps) in a new
  // Local Search in case of infeasibility
  if (!indiv->isFeasible &&
...
void Population::generatePopulation() {
  ...
  // we also reset it when resetting the population (probably not ideal but test
  // before changing)
  ...
  int nofRandomIndividualsToGenerate = std::max(
      0, nofIndividuals - nofNearestIndividualsToGenerate -
             nofFurthestIndividualsToGenerate - nofSweepIndividualsToGenerate);
  ...
  if (indiv->isFeasible &&
    ...
    if (indiv->myCostSol.penalizedCost <
      ...
      if (params->config.isDimacsRun) {
        // Since the controller may kill the script at any time, directly write
        // output
        ...
        // Print solution for processing by the controller (after output is
        // written since controller may terminate program!) Note: delay for
        // writing is negligible
...
void Population::removeWorstBiasedFitness(SubPopulation &pop) {
  ...
  for (int i = 1; i < static_cast<int>(pop.size()); i++) {
    // An averageBrokenPairsDistanceClosest equal to 0 indicates that a clone
    // exists
...
void Population::restart() {
  ...
  if (params->config.nbKeepFeasibleAfterRestart == 0) {
    for (Individual *indiv : feasibleSubpopulation) {
      delete indiv;
    }
    feasibleSubpopulation.clear();
    bestSolutionRestart = Individual();
  } else if (feasibleSubpopulation.size() >
             params->config.nbKeepFeasibleAfterRestart) {
    std::sort(feasibleSubpopulation.begin(), feasibleSubpopulation.end(),
              [](const Individual *ind1, const Individual *ind2) {
                return ind1->myCostSol.penalizedCost <
                       ind2->myCostSol.penalizedCost;
              });
    for (int i = params->config.nbKeepFeasibleAfterRestart,
             j = feasibleSubpopulation.size();
         i < j; ++i) {
      delete feasibleSubpopulation[i];
    }
    feasibleSubpopulation.resize(params->config.nbKeepFeasibleAfterRestart);
    std::cout << "----- KEEP " << feasibleSubpopulation.size()
              << " best feasible solutions" << std::endl;
    assert(bestSolutionRestart.myCostSol.penalizedCost ==
           feasibleSubpopulation[0]->myCostSol.penalizedCost);
  }
  if (params->config.nbKeepInfeasibleAfterRestart == 0) {
    for (Individual *indiv : infeasibleSubpopulation) {
      delete indiv;
    }
    infeasibleSubpopulation.clear();
  } else if (infeasibleSubpopulation.size() >
             params->config.nbKeepInfeasibleAfterRestart) {
    std::sort(infeasibleSubpopulation.begin(), infeasibleSubpopulation.end(),
              [](const Individual *ind1, const Individual *ind2) {
                return ind1->myCostSol.penalizedCost <
                       ind2->myCostSol.penalizedCost;
              });
    for (int i = params->config.nbKeepInfeasibleAfterRestart,
             j = infeasibleSubpopulation.size();
         i < j; ++i) {
      delete infeasibleSubpopulation[i];
    }
    infeasibleSubpopulation.resize(params->config.nbKeepInfeasibleAfterRestart);
    std::cout << "----- KEEP " << infeasibleSubpopulation.size()
              << " best infeasible solutions" << std::endl;
  }
...
void Population::printState(int nbIter, int nbIterNoImprovement) {
  ...
  // solutions, the cost of the best feasible solution, and the average cost of
  ...
  // Print the diversity of both pools of solutions, the average load- and time
  // warp feasibilities of the last 100 solutions generated by LS, and the
  // penalties for the capacit and the time warp
  ...
  std::printf(" | Pen %.2f %.2f", params->penaltyCapacity,
              params->penaltyTimeWarp);
  std::printf(" | nbG %d", params->config.nbGranular);
  std::cout << std::endl;
}
...
void Population::exportBKS(std::string fileName) {
  ...
  if (bestSolutionOverall.myCostSol.penalizedCost < 1.e29 &&
      (!readOK ||
       bestSolutionOverall.myCostSol.penalizedCost < readCost - MY_EPSILON)) {
...
void Population::exportSearchProgress(std::string fileName,
                                      std::string instanceName, int seedRNG) {
...
void Population::logSolution(int nbIter, std::ofstream &myfile,
                             Individual *indiv) {
...
Population::Population(Params *params, Split *split, LocalSearch *localSearch)
  ...
  // Create lists for the load feasibility of the last 100 individuals generated
  // by LS, where all feasibilities are set to true
