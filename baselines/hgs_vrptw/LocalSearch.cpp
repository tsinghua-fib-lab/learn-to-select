...
void LocalSearch::constructIndividualBySweep(int fillPercentage,
  ...
  for (int r = 0; r < static_cast<int>(nodeIndicesPerRoute.size()); r++) {
    ...
    for (int idx : nodeIndicesPerRoute[r]) {
      // Arbitrary division, but for all instances time windows are either much
      // shorter than half of depotOpeningDuration, or much larger.
    ...
    for (int i = 0; i < static_cast<int>(nodeIndicesToInsertLongTw.size());
      ...
      for (int j = 0; j <= routes[r].nbCustomers; j++) {
        ...
        double insertionCost =
            ...
            timeCost->get(nodesToInsert[nodeIndicesToInsertLongTw[i]].clientIdx,
                          prev->next->cour) -
            timeCost->get(prev->cour, prev->next->cour);
...
void LocalSearch::constructIndividualWithSeedOrder(
  ...
  // Construct routes
  for (int r = 0;
       r < static_cast<int>(routes.size()) && unassignedNodeIndices.size() > 0;
       r++) {
    ...
    if (useSeedClientFurthestFromDepot) {
      ...
      for (int idx : unassignedNodeIndices) {
        double insertionCost =
            timeCost->get(routes[r].depot->cour, nodesToInsert[idx].clientIdx) +
            timeCost->get(nodesToInsert[idx].clientIdx,
                          routes[r].depot->next->cour) -
            timeCost->get(routes[r].depot->cour, routes[r].depot->next->cour);
    ...
    while (insertedNode) {
      ...
      for (int idx : unassignedNodeIndices) {
        ...
        Node *prev = routes[r].depot;
        for (int j = 0; j <= routes[r].nbCustomers; j++) {
          // Do not allow insertions if time windows are violated more than
          // tolerance
          TimeWindowData routeTwData;
          MergeTWDataInto(routeTwData, prev->prefixTwData,
                          nodesToInsert[idx].twData, prev->next->postfixTwData);
          ...
          // Compute insertion cost
          double insertionCost =
              timeCost->get(prev->cour, nodesToInsert[idx].clientIdx) +
              timeCost->get(nodesToInsert[idx].clientIdx, prev->next->cour) -
              timeCost->get(prev->cour, prev->next->cour);
...
void LocalSearch::run(Individual *indiv, double penaltyCapacityLS,
  ...
  for (int i = 1; i <= params->nbClients; i++)
    if (params->rng() % params->config.nbGranular ==
        0) // Designed to use O(nbGranular x n) time overall to avoid possible
           // bottlenecks
  ...
  for (loopID = 0; !searchCompleted; loopID++) {
    if (loopID > 1) {
      // Allows at least two loops since some moves involving empty routes are
      // not checked at the first loop
    ...
    for (int posU = 0; posU < params->nbClients; posU++) {
      ...
      for (const auto &v : correlated) {
        ...
        if (loopID == 0 ||
            std::max(nodeU->route->whenLastModified,
                ...
                lastTestRINodeU) // only evaluate moves involving routes that
                                 // have been modified since last move
                                 // evaluations for nodeU
        {
          // Randomizing the order of the neighborhoods within this loop does
          // not matter much as we are already randomizing the order of the node
          // pairs (and it's not very common to find improving moves of
      ...
      /* MOVES INVOLVING AN EMPTY ROUTE -- NOT TESTED IN THE FIRST LOOP TO AVOID
       * INCREASING TOO MUCH THE FLEET SIZE */
...
bool LocalSearch::MoveSingleClient() {
  ...
  double costSuppU = timeCost->get(nodeUPrevIndex, nodeXIndex) -
                     timeCost->get(nodeUPrevIndex, nodeUIndex) -
                     timeCost->get(nodeUIndex, nodeXIndex);
  double costSuppV = timeCost->get(nodeVIndex, nodeUIndex) +
                     timeCost->get(nodeUIndex, nodeYIndex) -
                     timeCost->get(nodeVIndex, nodeYIndex);
  ...
  if (routeU != routeV) {
    ...
    MergeTWDataInto(routeUTwData, nodeU->prev->prefixTwData,
                    nodeX->postfixTwData);
    MergeTWDataInto(routeVTwData, nodeV->prefixTwData, nodeU->twData,
                    nodeY->postfixTwData);
    ...
    if (nodeU->position < nodeV->position) {
      ...
      MergeTWDataInto(routeUTwData, nodeV->prefixTwData, nodeU->twData,
...
bool LocalSearch::MoveTwoClients() {
  ...
  double costSuppU = timeCost->get(nodeUPrevIndex, nodeXNextIndex) -
                     timeCost->get(nodeUPrevIndex, nodeUIndex) -
                     timeCost->get(nodeXIndex, nodeXNextIndex);
  double costSuppV = timeCost->get(nodeVIndex, nodeUIndex) +
                     timeCost->get(nodeXIndex, nodeYIndex) -
                     timeCost->get(nodeVIndex, nodeYIndex);
  TimeWindowData routeUTwData, routeVTwData;
  ...
  if (routeU != routeV) {
    ...
    MergeTWDataInto(routeVTwData, nodeV->prefixTwData,
    ...
    // Move within the same route
    if (nodeU->position < nodeV->position) {
      // Edge case V directly after U, so X == V is excluded, V directly after X
      // so XNext == V works start - ... - UPrev - XNext - ... - V - U - X - Y -
      // ... - end
      MergeTWDataInto(routeUTwData, nodeU->prev->prefixTwData,
      ...
      MergeTWDataInto(routeUTwData, nodeV->prefixTwData,
...
bool LocalSearch::MoveTwoClientsReversed() {
  ...
  double costSuppU = timeCost->get(nodeUPrevIndex, nodeXNextIndex) -
                     ...
                     timeCost->get(nodeUIndex, nodeXIndex) -
                     timeCost->get(nodeXIndex, nodeXNextIndex);
  double costSuppV = timeCost->get(nodeVIndex, nodeXIndex) +
                     timeCost->get(nodeXIndex, nodeUIndex) +
                     timeCost->get(nodeUIndex, nodeYIndex) -
                     timeCost->get(nodeVIndex, nodeYIndex);
  TimeWindowData routeUTwData, routeVTwData;
  ...
  if (routeU != routeV) {
    ...
    if (nodeU->position < nodeV->position) {
      // Edge case V directly after U, so X == V is excluded, V directly after X
      // so XNext == V works start - ... - UPrev - XNext - ... - V - X - U - Y -
      // ... - end
      MergeTWDataInto(routeUTwData, nodeU->prev->prefixTwData,
                      getRouteSegmentTwData(nodeX->next, nodeV),
                      getEdgeTwData(nodeX, nodeU), nodeY->postfixTwData);
...
bool LocalSearch::SwapTwoSingleClients() {
  ...
  double costSuppU = timeCost->get(nodeUPrevIndex, nodeVIndex) +
                     timeCost->get(nodeVIndex, nodeXIndex) -
                     timeCost->get(nodeUPrevIndex, nodeUIndex) -
                     timeCost->get(nodeUIndex, nodeXIndex);
  double costSuppV = timeCost->get(nodeVPrevIndex, nodeUIndex) +
                     timeCost->get(nodeUIndex, nodeYIndex) -
                     timeCost->get(nodeVPrevIndex, nodeVIndex) -
                     timeCost->get(nodeVIndex, nodeYIndex);
  TimeWindowData routeUTwData, routeVTwData;
  ...
  if (routeU != routeV) {
    ...
    MergeTWDataInto(routeUTwData, nodeU->prev->prefixTwData, nodeV->twData,
                    nodeX->postfixTwData);
    MergeTWDataInto(routeVTwData, nodeV->prev->prefixTwData, nodeU->twData,
                    nodeY->postfixTwData);
    ...
    if (nodeU->position < nodeV->position) {
      // Edge case V directly after U, so X == V is excluded, V directly after X
      // so XNext == V works start - ... - UPrev - V - X - ... - VPrev - U - Y -
      // ... - end
      MergeTWDataInto(routeUTwData, nodeU->prev->prefixTwData, nodeV->twData,
                      getRouteSegmentTwData(nodeX, nodeV->prev), nodeU->twData,
                      nodeY->postfixTwData);
    } else {
      // Edge case U directly after V is excluded from beginning of function
      // start - ... - VPrev - U - Y - ... - UPrev - V - X - ... - end
      MergeTWDataInto(routeUTwData, nodeV->prev->prefixTwData, nodeU->twData,
...
bool LocalSearch::SwapTwoClientsForOne() {
  ...
  double costSuppU = timeCost->get(nodeUPrevIndex, nodeVIndex) +
                     timeCost->get(nodeVIndex, nodeXNextIndex) -
                     timeCost->get(nodeUPrevIndex, nodeUIndex) -
                     timeCost->get(nodeXIndex, nodeXNextIndex);
  double costSuppV = timeCost->get(nodeVPrevIndex, nodeUIndex) +
                     timeCost->get(nodeXIndex, nodeYIndex) -
  ...
  if (routeU != routeV) {
    ...
    MergeTWDataInto(routeUTwData, nodeU->prev->prefixTwData, nodeV->twData,
                    nodeX->next->postfixTwData);
    MergeTWDataInto(routeVTwData, nodeV->prev->prefixTwData,
                    getEdgeTwData(nodeU, nodeX), nodeY->postfixTwData);
    ...
    if (nodeU->position < nodeV->position) {
      ...
      MergeTWDataInto(routeUTwData, nodeV->prev->prefixTwData,
                      getEdgeTwData(nodeU, nodeX),
                      getRouteSegmentTwData(nodeY, nodeU->prev), nodeV->twData,
                      nodeX->next->postfixTwData);
    }
  ...
  // Note: next two lines are a bit inefficient but we only update occasionally
  // and updateRouteData is much more costly anyway, efficient checks are more
  // important
  swapNode(nodeU, nodeV);
  insertNode(nodeX, nodeU);
  nbMoves++; // Increment move counter before updating route data
...
bool LocalSearch::SwapTwoClientPairs() {
  ...
  double costSuppU = timeCost->get(nodeUPrevIndex, nodeVIndex) +
                     timeCost->get(nodeYIndex, nodeXNextIndex) -
                     timeCost->get(nodeUPrevIndex, nodeUIndex) -
                     timeCost->get(nodeXIndex, nodeXNextIndex);
  double costSuppV = timeCost->get(nodeVPrevIndex, nodeUIndex) +
                     timeCost->get(nodeXIndex, nodeYNextIndex) -
                     timeCost->get(nodeVPrevIndex, nodeVIndex) -
                     timeCost->get(nodeYIndex, nodeYNextIndex);
  TimeWindowData routeUTwData, routeVTwData;
...
bool LocalSearch::TwoOptWithinTrip() {
  ...
  double cost = timeCost->get(nodeUIndex, nodeVIndex) +
                timeCost->get(nodeXIndex, nodeYIndex) -
                timeCost->get(nodeUIndex, nodeXIndex) -
                timeCost->get(nodeVIndex, nodeYIndex) +
  ...
  Node *itRoute = nodeV;
  while (itRoute != nodeU) {
    MergeTWDataInto_(routeTwData, itRoute->twData);
    itRoute = itRoute->prev;
  }
  MergeTWDataInto_(routeTwData, nodeY->postfixTwData);
...
bool LocalSearch::TwoOptBetweenTrips() {
  double costSuppU = timeCost->get(nodeUIndex, nodeYIndex) -
                     timeCost->get(nodeUIndex, nodeXIndex);
  double costSuppV = timeCost->get(nodeVIndex, nodeXIndex) -
                     timeCost->get(nodeVIndex, nodeYIndex);
  ...
  MergeTWDataInto(routeUTwData, nodeU->prefixTwData, nodeY->postfixTwData);
  MergeTWDataInto(routeVTwData, nodeV->prefixTwData, nodeX->postfixTwData);
...
bool LocalSearch::swapStar(const bool withTW) {
  ...
  for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next) {
    for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next) {
      ...
      if (deltaLoadPen + deltaRemoval <= 0) {
        ...
        int extraV, extraU;
        if (withTW) {
          // Evaluate best reinsertion cost of U in the route of V where V has
          // been removed
          extraV = getCheapestInsertSimultRemovalWithTW(
              nodeU, nodeV, mySwapStar.bestPositionU);

          // Evaluate best reinsertion cost of V in the route of U where U has
          // been removed
          ...
          // Evaluate best reinsertion cost of U in the route of V where V has
          // been removed
          extraV = getCheapestInsertSimultRemoval(nodeU, nodeV,
                                                  mySwapStar.bestPositionU);

          // Evaluate best reinsertion cost of V in the route of U where U has
          // been removed
          extraU = getCheapestInsertSimultRemoval(nodeV, nodeU,
  ...
  double costSuppU =
      timeCost->get(myBestSwapStar.bestPositionV->cour,
                    myBestSwapStar.V->cour) -
      timeCost->get(myBestSwapStar.U->prev->cour, myBestSwapStar.U->cour) -
      timeCost->get(myBestSwapStar.U->cour, myBestSwapStar.U->next->cour);
  double costSuppV =
      timeCost->get(myBestSwapStar.bestPositionU->cour,
                    myBestSwapStar.U->cour) -
      timeCost->get(myBestSwapStar.V->prev->cour, myBestSwapStar.V->cour) -
      timeCost->get(myBestSwapStar.V->cour, myBestSwapStar.V->next->cour);

  if (myBestSwapStar.bestPositionV == myBestSwapStar.U->prev) {
    // Insert in place of U
    costSuppU +=
        timeCost->get(myBestSwapStar.V->cour, myBestSwapStar.U->next->cour);
  } else {
    costSuppU += timeCost->get(myBestSwapStar.V->cour,
                               myBestSwapStar.bestPositionV->next->cour) +
                 timeCost->get(myBestSwapStar.U->prev->cour,
  ...
  if (myBestSwapStar.bestPositionU == myBestSwapStar.V->prev) {
    ...
    costSuppV +=
        timeCost->get(myBestSwapStar.U->cour, myBestSwapStar.V->next->cour);
  } else {
    costSuppV += timeCost->get(myBestSwapStar.U->cour,
                               myBestSwapStar.bestPositionU->next->cour) +
                 timeCost->get(myBestSwapStar.V->prev->cour,
  ...
  TimeWindowData routeUTwData, routeVTwData;
  // It is not possible to have bestPositionU == V or bestPositionV == U, so the
  // positions are always strictly different
  if (myBestSwapStar.bestPositionV->position ==
      myBestSwapStar.U->position - 1) {
    // Special case
    MergeTWDataInto(routeUTwData, myBestSwapStar.bestPositionV->prefixTwData,
    ...
    MergeTWDataInto(routeUTwData, myBestSwapStar.U->prev->prefixTwData,
  ...
  if (myBestSwapStar.bestPositionU->position ==
    ...
    // Special case
    MergeTWDataInto(routeVTwData, myBestSwapStar.bestPositionU->prefixTwData,
    ...
    MergeTWDataInto(routeVTwData, myBestSwapStar.V->prev->prefixTwData,
...
bool LocalSearch::RelocateStar() {
  ...
  for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next) {
    ...
    const double costSuppU = timeCost->get(nodeUPrevIndex, nodeXIndex) -
                             timeCost->get(nodeUPrevIndex, nodeUIndex) -
                             timeCost->get(nodeUIndex, nodeXIndex) +
    ...
    for (Node *V = routeV->depot->next; !V->isDepot; V = V->next) {
      ...
      double costSuppV = timeCost->get(V->cour, nodeUIndex) +
                         timeCost->get(nodeUIndex, V->next->cour) -
                         timeCost->get(V->cour, V->next->cour) +
...
int LocalSearch::getCheapestInsertSimultRemoval(Node *U, Node *V,
                                                Node *&bestPosition) {
  ThreeBestInsert *myBestInsert = &bestInsertClient[V->route->cour][U->cour];
  bool found = false;

  // Find best insertion in the route such that V is not next or pred (can only
  // belong to the top three locations)
  ...
  int deltaCost = timeCost->get(V->prev->cour, U->cour) +
                  timeCost->get(U->cour, V->next->cour) -
                  timeCost->get(V->prev->cour, V->next->cour);
...
int LocalSearch::getCheapestInsertSimultRemovalWithTW(Node *U, Node *V,
                                                      Node *&bestPosition) {
  ...
  MergeTWDataInto(twData, V->prev->prefixTwData, U->twData,
...
void LocalSearch::preprocessInsertions(Route *R1, Route *R2) {
  for (Node *U = R1->depot->next; !U->isDepot; U = U->next) {
    ...
    U->deltaRemoval = timeCost->get(U->prev->cour, U->next->cour) -
                      timeCost->get(U->prev->cour, U->cour) -
                      timeCost->get(U->cour, U->next->cour);
    ...
    if (R2->whenLastModified > currentOption.whenLastCalculated) {
      ...
      currentOption.bestCost[0] =
          timeCost->get(0, U->cour) +
          timeCost->get(U->cour, R2->depot->next->cour) -
          timeCost->get(0, R2->depot->next->cour);
      currentOption.bestLocation[0] = R2->depot;
      for (Node *V = R2->depot->next; !V->isDepot; V = V->next) {
        int deltaCost = timeCost->get(V->cour, U->cour) +
                        timeCost->get(U->cour, V->next->cour) -
                        timeCost->get(V->cour, V->next->cour);
...
void LocalSearch::preprocessInsertionsWithTW(Route *R1, Route *R2) {
  ...
  for (Node *U = R1->depot->next; !U->isDepot; U = U->next) {
    // Performs the preprocessing
    // Note: when removing U and adding V to a route, the timewarp penalties may
    // interact, however in most cases it will hold that the reduced timewarp
    // from removing U + added timewarp from adding V will be bigger than the
    // actual delta timewarp such that assuming independence gives a
    // conservative estimate

    if (R1->isDeltaRemovalTWOutdated) {
      MergeTWDataInto(twData, U->prev->prefixTwData, U->next->postfixTwData);
      U->deltaRemovalTW = timeCost->get(U->prev->cour, U->next->cour) -
                          timeCost->get(U->prev->cour, U->cour) -
                          timeCost->get(U->cour, U->next->cour) +
    ...
    if (R2->whenLastModified > currentOption.whenLastCalculated) {
      ...
      // Compute additional timewarp we get when inserting U in R2, this may be
      // actually less if we remove U but we ignore this to have a conservative
      // estimate
      MergeTWDataInto(twData, R2->depot->prefixTwData, U->twData,
      ...
      currentOption.bestLocation[0] = R2->depot;
      for (Node *V = R2->depot->next; !V->isDepot; V = V->next) {
        MergeTWDataInto(twData, V->prefixTwData, U->twData,
                        V->next->postfixTwData);
        int deltaCost = timeCost->get(V->cour, U->cour) +
                        timeCost->get(U->cour, V->next->cour) -
                        timeCost->get(V->cour, V->next->cour) +
...
TimeWindowData LocalSearch::getRouteSegmentTwData(Node *U, Node *V) {
  ...
  while (!(mynode == V)) {
    if (mynode->isSeed && mynode->position + 4 <= targetPos) {
      MergeTWDataInto_(twData, mynode->toNextSeedTwD);
      mynode = mynode->nextSeed;
    } else {
      mynode = mynode->next;
      MergeTWDataInto_(twData, mynode->twData);
    }
  }
  return twData;
}
...
TimeWindowData
LocalSearch::MergeTWDataRecursive(const TimeWindowData &twData1,
                                  const TimeWindowData &twData2) {
  // Note, assume time equals cost
  int deltaDuration =
      timeCost->get(twData1.lastNodeIndex, twData2.firstNodeIndex);
  ...
  return TimeWindowData{
      .firstNodeIndex = twData1.firstNodeIndex,
      .lastNodeIndex = twData2.lastNodeIndex,
      .duration =
          twData1.duration + twData2.duration + deltaDuration + deltaWaitTime,
      .timeWarp = twData1.timeWarp + twData2.timeWarp + deltaTimeWarp,
      .earliestArrival =
...
void LocalSearch::MergeTWDataInto(TimeWindowData &twOut,
  ...
  int deltaTimeWarp =
      std::max(twData1.earliestArrival + delta - twData2.latestArrival, 0);
  twOut.firstNodeIndex = twData1.firstNodeIndex;
  twOut.lastNodeIndex = twData2.lastNodeIndex;
  twOut.duration =
      twData1.duration + twData2.duration + deltaDuration + deltaWaitTime;
  twOut.timeWarp = twData1.timeWarp + twData2.timeWarp + deltaTimeWarp;
  twOut.earliestArrival =
      std::max(twData2.earliestArrival - delta, twData1.earliestArrival) -
      deltaWaitTime;
  twOut.latestArrival =
      std::min(twData2.latestArrival - delta, twData1.latestArrival) +
...
void LocalSearch::MergeTWDataInto_(TimeWindowData &twData1,
                                   const TimeWindowData &twData2) {
  // Note, assume time equals cost
  int deltaDuration =
      timeCost->get(twData1.lastNodeIndex, twData2.firstNodeIndex);
...
void LocalSearch::MergeTWDataInto(TimeWindowData &twOut,
  ...
  MergeTWDataInto(twOut, twData1, twData2);
  MergeTWDataInto_(twOut, twData3);
}
...
void LocalSearch::MergeTWDataInto(TimeWindowData &twOut,
  ...
  MergeTWDataInto(twOut, twData1, twData2);
  MergeTWDataInto_(twOut, twData3);
  MergeTWDataInto_(twOut, twData4);
}
...
void LocalSearch::MergeTWDataInto(TimeWindowData &twOut,
  ...
  MergeTWDataInto_(twOut, twData3);
  MergeTWDataInto_(twOut, twData4);
  MergeTWDataInto_(twOut, twData5);
}
...
void LocalSearch::updateRouteData(Route *myRoute) {
  ...
  while (!mynode->isDepot || firstIt) {
    ...
    mytime += timeCost->get(mynode->prev->cour, mynode->cour) +
              params->cli[mynode->cour].serviceDuration;
    myReversalDistance += timeCost->get(mynode->cour, mynode->prev->cour) -
                          timeCost->get(mynode->prev->cour, mynode->cour);
    mynode->cumulatedLoad = myload;
    mynode->cumulatedTime = mytime;
    mynode->cumulatedReversalDistance = myReversalDistance;
    MergeTWDataInto(mynode->prefixTwData, mynode->prev->prefixTwData,
                    mynode->twData);
    mynode->isSeed = false;
    ...
    if (!mynode->isDepot) {
      ...
      if (myplace % 4 == 0) {
        if (seedNode != nullptr) {
          ...
          MergeTWDataInto(seedNode->toNextSeedTwD, seedTwD, mynode->twData);
        ...
        MergeTWDataInto_(seedTwD, mynode->twData);
      }
    }
    firstIt = false;
  }

  myRoute->duration = mytime; // Driving duration + service duration, excluding
                              // waiting time / time warp
  myRoute->load = myload;
  ...
  while (!mynode->isDepot || firstIt) {
    mynode = mynode->prev;
    MergeTWDataInto(mynode->postfixTwData, mynode->twData,
                    mynode->next->postfixTwData);
    firstIt = false;
  }
...
void LocalSearch::exportIndividual(Individual *indiv) {
  ...
  std::sort(
      ...
      routePolarAngles.end()); // empty routes have a polar angle of 1.e30, and
...
LocalSearch::LocalSearch(Params *params)
    : params(params), timeCost(&params->timeCost) {
