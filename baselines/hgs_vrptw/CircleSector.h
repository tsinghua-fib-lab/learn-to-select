...
struct CircleSector {
  ...
  static int positive_mod(int i) {
    ...
    // Remark that "i % 65536" should be automatically compiled in an optimized
    // form as "i & 0xffff" for faster calculations
  ...
  static bool overlap(const CircleSector &sector1, const CircleSector &sector2,
                      const int tolerance) {
    // the RHS is the size of the sector, by adding the tolerance outside of the
    // positive_mod, we avoid overflow beyond a full circle
  ...
  void extend(int point) {
    ...
    if (!isEnclosed(point)) {
      // If the point is outside the circle sector, extend the circle sector in
      // a greedy way
