...
void Params::readInput(std::istream &inputFile) {
  ...
  if (content.substr(0, 7) == "VEHICLE") {
    ...
    // Create a vector where all information on the Clients can be stored and
    // loop over all information in the file
    ...
    while (inputFile >> node) {
      ...
      inputFile >> cli[nbClients].coordX >> cli[nbClients].coordY >>
          ...
          cli[nbClients].latestArrival >> cli[nbClients].serviceDuration;

      // Scale coordinates by factor 10, later the distances will be rounded
      // so we optimize with 1 decimal distances
      ...
      // Keep track of the max demand, the total demand, and the number of
      // clients
    ...
    // Check if the required service and the start of the time window of the
    // depot are both zero
    ...
    for (inputFile >> content; content != "EOF"; inputFile >> content) {
      ...
      // Read the type of edge weights
        ...
        if (!isExplicitDistanceMatrix) {
          throw std::string("EDGE_WEIGHT_FORMAT can only be used with "
      ...
      // Read the data on the service time (used when the service time is
      ...
      // Read the edge weights of an explicit distance matrix
        ...
        for (int i = 0; i <= nbClients; i++) {
          for (int j = 0; j <= nbClients; j++) {
            // Keep track of the largest distance between two clients (or the
            // depot)
            int cost;
            inputFile >> cost;
            if (cost > maxDist) {
      ...
      // Read the demand of each client (including the depot, which should
      // have demand 0)
        ...
        for (int i = 0; i <= nbClients; i++) {
          ...
          if (clientNr != i + 1) {
            throw std::string(
                "Clients are not in order in the list of demands");
          }
      ...
      // Read the time windows of all the clients (the depot should have a
      // time window from 0 to max)
...
Params::Params(const CommandLine &cl) {
  ...
  // Convert the circle sector parameters from degrees ([0,359]) to [0,65535] to
  ...
  if (config.isDimacsRun) {
    ...
    std::cout << "DIMACS RUN for instance name " << instanceName
              << ", writing solution to " << config.pathSolution << std::endl;
  }
  ...
  if (maxDemand < 0.1 || maxDemand > 100000) {
    throw std::string("The demand quantities are of very small or large scale. "
                      "This could impact numerical stability. Please rescale "
                      "the dataset and run again.");
  }
  ...
  penaltyCapacity =
      std::max(0.1, std::min(1000., static_cast<double>(maxDist) / maxDemand));
...
void Params::SetCorrelatedVertices() {
  ...
  // First create a set of correlated vertices for each vertex (where the depot
  ...
  for (int i = 1; i <= nbClients; i++) {
    ...
    // Loop over all clients (taking into account the max number of clients and
    // the granular restriction)
    for (int j = 0; j < std::min(config.nbGranular, nbClients - 1); j++) {
      // If i is correlated with j, then j should be correlated with i (unless
      // we have asymmetric problem with time windows) Insert vertices in
      // setCorrelatedVertices, in the order of orderProximity, where .second is
