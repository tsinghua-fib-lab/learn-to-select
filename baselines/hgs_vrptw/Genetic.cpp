...
void Genetic::run(int maxIterNonProd, int timeLimit) {
  if (params->nbClients == 1) {
    // Edge case: with 1 client, crossover will fail, genetic algorithm makes no
    // sense
    return;
  }
  // Do iterations of the Genetic Algorithm, until more then maxIterNonProd
  // consecutive iterations without improvement or a time limit (in seconds) is
  // reached
  // 重复规则：如果到了maxIterNonProd，并且剩余时间够重复一次且doRepeatUntilTimeLimit的情况下才重复
  int nbIterNonProd = 1;
  std::vector<double> epoch_time; // 记录每次用时
  double epoch_start_time = params->getTimeElapsedSeconds();
  for (int nbIter = 0; !params->isTimeLimitExceeded(); nbIter++) {
    ...
    if (!offspring->isFeasible &&
      ...
      // Run the Local Search again, but with penalties for infeasibilities
      // multiplied by 10
      localSearch->run(offspring, params->penaltyCapacity * 10.,
                       params->penaltyTimeWarp * 10.);
      // If the individual is feasible now, check if it is the best feasible
      // individual of the population, based on penalizedCost and add it to the
      // population If the individual is not feasible now, it is not added to
      // the population
      if (offspring->isFeasible) {
    ...
    if (params->config.logPoolInterval > 0 &&
      ...
      population->exportPopulation(nbIter,
                                   params->config.pathSolution + ".log.csv");
    }
    ...
    if (nbIterNonProd == maxIterNonProd) {
      if (!params->config.doRepeatUntilTimeLimit) {
        break;
      }
      auto t = params->getTimeElapsedSeconds();
      epoch_time.push_back(t - epoch_start_time);
      epoch_start_time = t;
      double mean = 0;
      for (auto i : epoch_time) {
        mean += i;
      }
      mean /= epoch_time.size();
      if (params->getTimeElapsedSeconds() + mean < params->config.timeLimit) {
        if (params->config.nbKeepFeasibleAfterRestart != 0) {
          maxIterNonProd += nbIterNonProd;
        }
        population->restart();
        nbIterNonProd = 1;
      }
    ...
    // vertices again) every certain number of iterations, if growNbGranularSize
    // is greater than 0
    ...
    // Increase the minimumPopulationSize by growPopulationSize every certain
    // number of iterations, if growPopulationSize is greater than 0
...
Individual *Genetic::crossoverSREX(
  ...
  while (improved) {
    ...
    const int differenceALeft =
        ...
        static_cast<int>(std::count_if(
            parents.first->chromR[(startA + nOfMovedRoutes - 1) % nOfRoutesA]
                .begin(),
            parents.first->chromR[(startA + nOfMovedRoutes - 1) % nOfRoutesA]
                .end(),
            [&clientsInSelectedB](int c) {
    ...
    const int differenceBLeft =
        static_cast<int>(std::count_if(
            parents.second->chromR[(startB - 1 + nOfMovedRoutes) % nOfRoutesB]
                .begin(),
            parents.second->chromR[(startB - 1 + nOfMovedRoutes) % nOfRoutesB]
                .end(),
  ...
  std::copy_if(
      clientsInSelectedA.begin(), clientsInSelectedA.end(),
  ...
  std::copy_if(
      clientsInSelectedB.begin(), clientsInSelectedB.end(),
  ...
  // Step 3: Insert unplanned clients (those that were in the removed routes of
  // A but not the inserted routes of B)
...
void Genetic::insertUnplannedTasks(Individual *offspring,
  ...
  for (int c : unplannedTasks) {
    ...
    for (int r = 0; r < params->nbVehicles; r++) {
      ...
      auto &chrom = offspring->chromR[r];
      if (chrom.empty()) {
        continue;
      }

      newDistanceFromInsert = timeCost->get(c, chrom[0]);
      if (earliestArrival + newDistanceFromInsert <
          params->cli[chrom[0]].latestArrival) {
        distanceDelta = timeCost->get(0, c) + newDistanceToInsert -
                        timeCost->get(0, chrom[0]);
      ...
      for (int i = 1, j = static_cast<int>(chrom.size()); i < j; i++) {
        newDistanceToInsert = timeCost->get(chrom[i - 1], c);
        newDistanceFromInsert = timeCost->get(c, chrom[i]);
        if (params->cli[chrom[i - 1]].earliestArrival + newDistanceToInsert <
                latestArrival &&
            earliestArrival + newDistanceFromInsert <
                params->cli[chrom[i]].latestArrival) {
          distanceDelta = newDistanceToInsert + newDistanceFromInsert -
                          timeCost->get(chrom[i - 1], chrom[i]);
          if (distanceDelta < bestDistance) {
            bestDistance = distanceDelta;
            bestLocation = {r, i};
          }
      ...
      newDistanceToInsert = timeCost->get(chrom.back(), c);
      if (params->cli[chrom.back()].earliestArrival + newDistanceToInsert <
          latestArrival) {
        distanceDelta = newDistanceToInsert + timeCost->get(c, 0) -
                        timeCost->get(chrom.back(), 0);
        if (distanceDelta < bestDistance) {
          bestDistance = distanceDelta;
          bestLocation = {r, static_cast<int>(chrom.size())};
        }
      }
    }
...
Genetic::Genetic(Params *params, Split *split, Population *population,
    ...
    : params(params), timeCost(&params->timeCost), split(split),
      population(population), localSearch(localSearch) {
  // After initializing the parameters of the Genetic object, also generate new
  // individuals in the array candidateOffsprings
